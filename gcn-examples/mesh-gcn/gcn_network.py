...
import torch_geometric.nn.unpool.knn_interpolate as knninterp
...
class GCNBlock(nn.Module):
    ...
    def forward_op(self, adj, data):
        ...
        x2_reshape = x2.transpose(0,1).reshape(x2.shape[1], -1)

        x3 = knninterp(x2_reshape, self.domain, self.range)

        x3_reshape = x3.reshape(-1, x2.shape[0], x2.shape[2]).transpose(0,1)

        sq_shape = int(np.sqrt(x3_reshape.shape[1]))

        dim_pack = [sq_shape] * self.spatial_dim

        output = self.resample(x3_reshape.reshape(x3_reshape.shape[0], *dim_pack, x3_reshape.shape[2])).reshape(x3_reshape.shape[0], -1, x3_reshape.shape[2])
    ...
    def adjoint_op(self, adj, data):
        ...
        x = self.resample(data.reshape(data.shape[0], *dim_pack, data.shape[2])).reshape(data.shape[0], -1, data.shape[2])

        x_reshape = x.transpose(0,1).reshape(x.shape[1], -1)

        x0 = knninterp(x_reshape, self.domain, self.range)

        x0_reshape = x0.reshape(-1, x.shape[0], x.shape[2]).transpose(0,1)

        x1 = self.conv1(x0_reshape, adj)
        x1 = self.activation1(self.batchnorm1(x1))

        x2 = self.conv2(x1, adj)
        x2 = self.activation2(self.batchnorm2(x2) + x0_reshape)
